...
export const accountsSchema = Type.Object(
    {
        ...
        orderIds: Type.Optional(Type.Array(ObjectIdSchema())),
...
export const accountsResolver = resolve({
    ...
    orders: virtual(async (account, context) => {
        const {data} = await context.app.service('orders').find({
            query: {
                _id: {
                    $in: account.orderIds || []
                }
...
export const accountsDataSchema = Type.Pick(accountsSchema, ['name', 'gender', 'age', 'userIds', 'orderIds'], {
    $id: 'AccountsData'
})
...
export const accountsQueryProperties = Type.Pick(accountsSchema, ['_id', 'name', 'gender', 'age', 'userIds', 'orderIds'])
